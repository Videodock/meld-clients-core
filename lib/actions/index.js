...
const FETCH_TARGET_EXPRESSION = 'FETCH_TARGET_EXPRESSION';
exports.FETCH_TARGET_EXPRESSION = FETCH_TARGET_EXPRESSION;
const FETCH_COMPONENT_TARGET = 'FETCH_COMPONENT_TARGET';
exports.FETCH_COMPONENT_TARGET = FETCH_COMPONENT_TARGET;
const PROCESS_COMPONENT_TARGET = 'PROCESS_COMPONENT_TARGET';
...
const FETCH_MANIFESTATIONS = 'FETCH_MANIFESTATIONS';
exports.FETCH_MANIFESTATIONS = FETCH_MANIFESTATIONS;
const SCORE_PREV_PAGE = 'SCORE_PREV_PAGE';
exports.SCORE_PREV_PAGE = SCORE_PREV_PAGE;
const SCORE_NEXT_PAGE = 'SCORE_NEXT_PAGE';
exports.SCORE_NEXT_PAGE = SCORE_NEXT_PAGE;
const SCORE_PAGE_TO_TARGET = 'SCORE_PAGE_TO_TARGET';
exports.SCORE_PAGE_TO_TARGET = SCORE_PAGE_TO_TARGET;
const PROCESS_ANNOTATION = 'PROCESS_ANNOTATION';
exports.PROCESS_ANNOTATION = PROCESS_ANNOTATION;
const SESSION_GRAPH_ETAG = 'SESSION_GRAPH_ETAG';
exports.SESSION_GRAPH_ETAG = SESSION_GRAPH_ETAG;
const RESET_NEXT_SESSION_TRIGGER = 'RESET_NEXT_SESSION_TRIGGER';
exports.RESET_NEXT_SESSION_TRIGGER = RESET_NEXT_SESSION_TRIGGER;
const TRANSITION_TO_NEXT_SESSION = 'TRANSITION_TO_NEXT_SESSION';
exports.TRANSITION_TO_NEXT_SESSION = TRANSITION_TO_NEXT_SESSION;
const REGISTER_PUBLISHED_PERFORMANCE_SCORE = 'REGISTER_PUBLISHED_PERFORMANCE_SCORE';
exports.REGISTER_PUBLISHED_PERFORMANCE_SCORE = REGISTER_PUBLISHED_PERFORMANCE_SCORE;
const MUZICODES_UPDATED = 'MUZICODES_UPDATED';
exports.MUZICODES_UPDATED = MUZICODES_UPDATED;
const REALIZATION_OF = 'frbr:realizationOf';
exports.REALIZATION_OF = REALIZATION_OF;
const EXPRESSION = 'frbr:Expression';
exports.EXPRESSION = EXPRESSION;
const PART_OF = 'frbr:partOf';
exports.PART_OF = PART_OF;
const PART = 'frbr:part';
exports.PART = PART;
const KEY = 'mo:key';
exports.KEY = KEY;
const HARMONY = 'https://meld.linkedmusic.org/companion/vocab/harmony';
exports.HARMONY = HARMONY;
const CADENCE = 'https://meld.linkedmusic.org/companion/vocab/cadentialGoal';
exports.CADENCE = CADENCE;
const DEGREE = 'https://meld.linkedmusic.org/companion/vocab/hasDegree';
exports.DEGREE = DEGREE;
const CHORD_TYPE = 'https://meld.linkedmusic.org/companion/vocab/chordType';
exports.CHORD_TYPE = CHORD_TYPE;
const HAS_STRUCTURE = 'https://meld.linkedmusic.org/terms/hasStructure';
exports.HAS_STRUCTURE = HAS_STRUCTURE;
const SEQ = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#Seq';
exports.SEQ = SEQ;
const SEQPART = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#_';
exports.SEQPART = SEQPART;
const SCORE = 'http://purl.org/ontology/mo/Score';
exports.SCORE = SCORE;
const CONTAINS = 'http://www.w3.org/ns/ldp#contains';
exports.CONTAINS = CONTAINS;
const MOTIVATED_BY = 'http://www.w3.org/ns/oa#motivatedBy';
exports.MOTIVATED_BY = MOTIVATED_BY;
const SEGMENT = 'so:Segment';
exports.SEGMENT = SEGMENT;
const MUZICODE = 'meld:Muzicode';
exports.MUZICODE = MUZICODE;
const PUBLISHED_AS = 'http://purl.org/ontology/mo/published_as';
exports.PUBLISHED_AS = PUBLISHED_AS;
const HAS_PERFORMANCE_MEDIUM = 'http://rdaregistry.info/Elements/e/p20215';
...
const TICK = "TICK";
exports.TICK = TICK;
const TRAVERSAL_PREHOP = "TRAVERSAL_PREHOP";
exports.TRAVERSAL_PREHOP = TRAVERSAL_PREHOP;
const TRAVERSAL_HOP = "TRAVERSAL_HOP";
...
const UPDATE_LATEST_RENDERED_PAGENUM = "UPDATE_LATEST_RENDERED_PAGENUM";
exports.UPDATE_LATEST_RENDERED_PAGENUM = UPDATE_LATEST_RENDERED_PAGENUM;
const muzicodesUri = "http://127.0.0.1:5000/MUZICODES";
exports.muzicodesUri = muzicodesUri;
const MAX_RETRIES = 3;
exports.MAX_RETRIES = MAX_RETRIES;
const MAX_TRAVERSAL_HOPS = 10;
exports.MAX_TRAVERSAL_HOPS = MAX_TRAVERSAL_HOPS;
const RETRY_DELAY = 10; // TODO move context somewhere global -- most framing happens server side
...
function fetchRibbonContent(url) {
  ...
  const promise = _solidAuthClient.default.fetch(url);
...
function registerTraversal(docUri, {
  ...
  objectUriBlacklist = [],
  objectTypeBlacklist = [],
  propertyPrefixWhitelist = [],
  propertyUriWhitelist = [],
  propertyPrefixBlacklist = [],
  propertyUriBlacklist = [],
  objectives = {},
...
function traverse(docUri, params) {
  ...
  const promise = _solidAuthClient.default.fetch(docUri, {
    headers: headers,
    mode: 'cors'
  });
  ...
  return dispatch => {
    ...
    promise.then(response => {
      ...
      } else if (docUri.endsWith(".ttl") || docUri.endsWith(".n3") || docUri.endsWith(".rdf") || docUri.endsWith(".nq") || docUri.endsWith(".nt") || response.headers.get("Content-Type").startsWith("application/rdf+xml") || response.headers.get("Content-Type").startsWith("application/nquads") || response.headers.get("Content-Type").startsWith("application/x-turtle") || response.headers.get("Content-Type").startsWith("text/turtle")) {// treat as RDF document
...
function traverseJSONLD(dispatch, docUri, params, dataPromise) {
  console.log("in traverseJSONLDf or doc ", docUri, "with blacklist ", params["objectUriBlacklist"]); // expand the JSON-LD object so that we are working with full URIs, not compacted into prefixes

  dataPromise.then(data => {
    console.log("attempting to expand: ", data);

    _jsonld.default.expand(data, (err, expanded) => {
      if (err) {
      ...
      _jsonld.default.flatten(expanded, (err, flattened) => {
...
function passesTraversalConstraints(obj, params) {
  ...
  if (params["objectUriWhitelist"].length) {
    ...
    if (!params["objectUriWhitelist"].includes(resourceUri) || params["objectUriBlacklist"].includes(resourceUri)) {
...
function checkTraversalObjectives(graph, objectives) {
  ...
  return dispatch => {
    objectives.map((obj, ix) => {
      _jsonld.default.frame(graph, obj, (err, framed) => {
        if (err) {
...
function fetchSessionGraph(uri, etag = "") {
  ...
  const promise = _solidAuthClient.default.fetch(uri, {
    headers: {
      'Accept': 'application/ld+json',
      'If-None-Match': etag
    },
...
function fetchComponentTarget(uri, conceptualScore = "") {
  ...
  return dispatch => {
    promise.then(data => {
      ...
      if (!"content-type" in data.headers || data.headers.get("Content-Type") !== "application/json" && data.headers.get("Content-Type") !== "application/ld+json") {
        ...
        _jsonld.default.fromRDF(data.data, (err, doc) => {
          if (err) {
  ...
  return dispatch => {
    _jsonld.default.frame(data, {
      ...
      if (err) {
        ...
        _jsonld.default.compact(framed, context, (err, compacted) => {
          if (err) {
...
function fetchWork(target, parts, work, expressionObj) {
  ...
  return dispatch => {
    ...
    _solidAuthClient.default.fetch(work).then(data => {
      _jsonld.default.fromRDF(data.data, (err, doc) => {
        if (err) {
...
function fetchStructure(target, parts, segline) {
  return dispatch => {
    ...
    _solidAuthClient.default.fetch(segline).then(data => {
      _jsonld.default.fromRDF(data.data, (err, doc) => {
        if (err) {
          console.log("ERROR TRANSLATING NQUADS TO JSONLD: ", err, data.data);
        } else {
...
function postAnnotation(session, etag, json, retries = MAX_RETRIES) {
  return dispatch => {
    if (retries) {
      ...
      _solidAuthClient.default.fetch(session, {
        ...
        if (error.response.status == 412) {
          ...
          _solidAuthClient.default.fetch(session).then(response => {
            return dispatch => {
              // and try again
              setTimeout(() => {
...
function markAnnotationProcessed(session, etag, annotation, retries = MAX_RETRIES) {
  if (retries) {
...
function patchAndProcessAnnotation(action, session, etag, annotation, success = {
  type: _meldActions.ANNOTATION_PATCHED
}, retries = MAX_RETRIES) {
  if (retries) {
    // console.log("PATCHING: ", session, etag, annotation);
...
function updateMuzicodes(muzicodesUri, session, mei = "") {
...
function createSession(sessionsUri, scoreUri, {
  ...
  return dispatch => {
    if (retries) {
      ...
      _solidAuthClient.default.fetch(sessionsUri).then(getResponse => {
